...
class TransVideo():
    def __init__(self,src_dir='./hmdb51_org',target_dir='./img51_org',
                 align_dir='./align51_org',test_sample_dir='./test51_org',
                 frames_len=25):
    ...
    def get_Interval(self,true_frames):
        rate=int(true_frames/self.frames_len)     #round
        if rate>=1:
            return rate
        else:
            return int(1)
    ...
    def test_Division(self,sample_limit=90):
        ...
        for i in range(align_51_len):
            ...
            align_sample_list=np.array(os.listdir(align_51_path))#[sample_limit:sample_limit+11]
            align_sample_list=align_sample_list[np.random.permutation(sample_limit)[:11]].tolist() #模拟随机抽取
            align_sample_len=len(align_sample_list)
    ...
    def copy_File(self,source_dir='./extend_org/align51_org',target_dir='./align51_org'):
